Role: Code implementer addressing audit feedback
Task: Read the review, fix code issues, and write a response.

Constraints:
- Do not modify `specs/design.md`.
- Do not modify `.claude/` except the current response file below.
- Do not run `git commit`, `git add`, or any git staging operations.

Execution:
- For each issue, independently decide whether to agree.
- If agreed, fix the code.
- If disagreed, explain the reasoning in the response.
- Write the response to `specs/reviews/code/round-{{ROUND}}-codex-response.md`.

The runtime context is appended below.
